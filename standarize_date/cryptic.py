from fuzzywuzzy import fuzz
from fuzzywuzzy import process
import json
import re
import csv

def standard_date(file_name,column_name):
  # data_compare ={
  #   "&": "AND",
  #   "1ST": "FIRST",
  #   "2ND": "SECOND",
  #   "3RD": "THIRD",
  #   "ABATE": "ABATEMENT",
  #   "ACAD": "ACADEMIC",
  #   "ACCT": "ACCOUNT",
  #   "ACK": "ACKNOWLEDGEMENT",
  #   "ACQ": "ACQUISITION",
  #   "ACT": "ACTIVITY",
  #   "ADMIN": "ADMINISTRATIVE",
  #   "ADMISS": "ADMISSION",
  #   "ADVNCD": "ADVANCED",
  #   "AIT": "ADVANCED",
  #   "ADV": "ADVISOR",
  #   "AFF": "AFFAIRS",
  #   "AA": "AFFIRMATIVE",
  #   "AIM": "AFTERCARE",
  #   "ALUM": "ALUMNI",
  #   "AMBUL": "AMBULATORY",
  #   "ADA": "AMERICAN",
  #   "ANGIO": "ANGIOGRAPHY",
  #   "ANN": "ANNUAL",
  #   "APPL": "APPLICATION",
  #   "APPT": "APPOINTMENT",
  #   "APR": "APPRENTICE",
  #   "A/S": "ARTS",
  #   "ASBES": "ASBESTOS",
  #   "ASSESS": "ASSESS",
  #   "ASST": "ASSISTANT",
  #   "AVC": "ASSISTANT/ASSOCIATE",
  #   "AVP": "ASSISTANT/ASSOCIATE",
  #   "AI": "ASSOCIATE",
  #   "ASSOC": "ASSOCIATE",
  #   "ABFE": "ASSOCIATION",
  #   "ASSUR": "ASSURANCE",
  #   "ATH": "ATHLETIC",
  #   "ATTD": "ATTENDANT",
  #   "AV": "AUDIO",
  #   "AUD": "AUDIOLOGY",
  #   "ACE/IVR": "AUTOMATED",
  #   "AUX": "AUXILIARY",
  #   "BSW": "BACHELOR",
  #   "BASBL": "BASEBALL",
  #   "BSKTB": "BASKETBALL",
  #   "BIOINFO": "BIOINFORMATICS",
  #   "BTHER": "BIOLOGIC",
  #   "BL": "BLOOMINGTON",
  #   "BLDG": "BUILDING",
  #   "BUS": "BUSINESS",
  #   "CAMP": "CAMPAIGN",
  #   "CNCR": "CANCER",
  #   "CAP": "CAPITAL",
  #   "CAM": "CAPITAL",
  #   "CLS": "CARDIAC",
  #   "CARDIO": "CARDIOLOGY",
  #   "CPR": "CARDIOPULMONARY",
  #   "CTLG": "CATALOGING",
  #   "CATH": "CATHERIZATION",
  #   "CAV": "CAVANAUGH",
  #   "CTR": "CENTER",
  #   "CRTN": "CERTIFICATION",
  #   "CHANC": "CHANCELLOR",
  #   "CHEM": "CHEMISTRY",
  #   "CEO": "CHIEF",
  #   "CFO": "CHIEF",
  #   "CHLDHD": "CHILDHOOD",
  #   "CIRC": "CIRCULATION",
  #   "CL": "CLASS",
  #   "CLSF": "CLASSIFY",
  #   "CLRGHOUSE": "CLEARNINGHOUSE",
  #   "CLIN": "CLINIC",
  #   "COLL": "COLLECTION",
  #   "CPI": "COLLEGE",
  #   "CMTE": "COMMITTEE",
  #   "COMM": "COMMUNICATION",
  #   "CMTY": "COMMUNITY",
  #   "COMPENS": "COMPENSATION",
  #   "CMPL": "COMPLIANCE",
  #   "CBC": "COMPOSITION",
  #   "CADD": "COMPUTER",
  #   "COMP": "COMPUTER",
  #   "CONF": "CONFERENCE",
  #   "CONST": "CONSTRUCTION",
  #   "CONS": "CONSULTANT",
  #   "CONSMR": "CONSUMER",
  #   "CONTIN": "CONTINUING",
  #   "CME": "CONTINUING",
  #   "CONT": "CONTROL",
  #   "COOP": "COOPERATIVE",
  #   "COORD": "COORDINATOR",
  #   "COUN": "COUNSELOR",
  #   "CURR": "CURRICULUM",
  #   "CYCL": "CYCLOTRON",
  #   "CYTO": "CYTOGENETICS",
  #   "DFAC": "DEAN",
  #   "DECIS": "DECISION",
  #   "DEL": "DELIVERY",
  #   "DENT": "DENTAL",
  #   "DEPT": "DEPARTMENT",
  #   "DPND": "DEPENDENCE",
  #   "DSGN": "DESIGN",
  #   "DEVL": "DEVELOPMENT",
  #   "DIAG": "DIAGNOSIS",
  #   "DIG": "DIGITAL",
  #   "DIR": "DIRECTOR",
  #   "DISAB": "DISABLED,",
  #   "DIST": "DISTANCE",
  #   "DISTRIB": "DISTRIBUTION",
  #   "DIVR": "DIVERSITY",
  #   "DIV": "DIVISION",
  #   "DOC": "DOCUMENT",
  #   "Dr.": "DOCTOR",
  #   "DUPL": "DUPLICATING",
  #   "ECG": "ECHOCARDIOGRAM",
  #   "EDUC": "EDUCATION",
  #   "EDUCR": "EDUCATOR",
  #   "ELEC": "ELECTRICAL",
  #   "EEG": "ELECTROENCEPHALOGRAPHIC",
  #   "ELECT": "ELECTRON",
  #   "ERA": "ELECTRONIC",
  #   "ELECTROPHYS": "ELECTROPHYSIOLOGY",
  #   "ELEM": "ELEMENTARY",
  #   "EMPL": "EMPLOYMENT",
  #   "ERCP": "ENDOSCOPIC",
  #   "ENFORCEMNT": "ENFORCEMENT",
  #   "ENG": "ENGINEER",
  #   "ES": "ENROLLMENT",
  #   "ENVIRO": "ENVIRONMENT",
  #   "EQUIP": "EQUIPMENT",
  #   "ERIC/CHESS": "ERIC",
  #   "EVAL": "EVALUATION",
  #   "EVEN": "EVENING",
  #   "EP": "EVOKED",
  #   "EXCEL": "EXCELLENCE",
  #   "EXEC": "EXECUTIVE",
  #   "EXER": "EXERCISE",
  #   "EXP": "EXPERIENCE",
  #   "FAC": "FACILITY",
  #   "FACET": "FACULTY",
  #   "FEL": "FELLOW",
  #   "FMS": "FINANCIAL",
  #   "FIS": "FISCAL",
  #   "FSER": "FOOD",
  #   "FOUNDTN": "FOUNDATION",
  #   "FRSH": "FRESHMAN",
  #   "FUTR": "FUTURE",
  #   "GI": "GASTROINTESTINAL",
  #   "GEN": "GENERAL",
  #   "GSN": "GENERAL",
  #   "GIS": "GEOGRAPHIC",
  #   "GENI": "GEOGRAPHY",
  #   "GIV": "GIVING",
  #   "GOVT": "GOVERNMENT",
  #   "GR": "GRADE",
  #   "GRAD": "GRADUATE",
  #   "GRND": "GROUND",
  #   "GRP": "GROUP",
  #   "HD": "HEAD",
  #   "HLTH": "HEALTH",
  #   "HIS": "HEALTH",
  #   "HVAC": "HEATING",
  #   "HEMA": "HEMATOLOGY",
  #   "HS": "HIGH",
  #   "HORTI": "HORTICULTURIST",
  #   "HOSP": "HOSPITAL",
  #   "HFC": "HOUSING",
  #   "HIV": "HUMAN",
  #   "HR": "HUMAN",
  #   "IMAG": "IMAGIN",
  #   "IMMUN": "IMMUNIZATION",
  #   "IN": "INDIANA",
  #   "IAI": "INDIANA",
  #   "IPCRES": "INDIANA",
  #   "IU": "INDIANA",
  #   "INDPLS": "INDIANAPOLIS",
  #   "IAHP": "INDIANAPOLIS",
  #   "INFEC": "INFECTION",
  #   "IBD": "INFLAMMATORY",
  #   "INFO": "INFORMATION",
  #   "IO": "INFORMATION",
  #   "IOCM": "INFORMATION",
  #   "INIT": "INITIATIVES",
  #   "INSP": "INSPECTOR",
  #   "INSTALL": "INSTALL",
  #   "INST": "INSTITUTION",
  #   "INSTR": "INSTRUCTION",
  #   "IMS": "INSTRUCTIONAL",
  #   "IT": "INSTRUCTIONAL",
  #   "INT": "INTERIM",
  #   "IAVP": "INTERIM",
  #   "INTERLIBR": "INTERLIBRARY",
  #   "INTNL": "INTERNATIONAL",
  #   "INTRN": "INTERNSHIP",
  #   "INTRREL": "INTERRELATIONS",
  #   "INTRM": "INTRAMURAL",
  #   "INV": "INVASIVE",
  #   "INVEST": "INVESTIGATIONS",
  #   "JR": "JUNIOR",
  #   "LAB": "LABORATORY",
  #   "LANG": "LANGUAGE",
  #   "LD": "LEAD",
  #   "LEAD": "LEADER",
  #   "LRNG": "LEARNING",
  #   "LEC": "LECTURER",
  #   "LAS": "LIBERAL",
  #   "LIBR": "LIBRARY",
  #   "LIC": "LICENSED",
  #   "LAN": "LOCAL",
  #   "MACH": "MACHINE",
  #   "MRI": "MAGNETIC",
  #   "MAINT": "MAINTENANCE",
  #   "MMS": "MAINTENANCE",
  #   "MBDC": "MAJOR",
  #   "MAMMO": "MAMMOGRAPHY",
  #   "MGT": "MANAGEMENT",
  #   "MGR": "MANAGER",
  #   "MNGNG": "MANAGING",
  #   "MKTG": "MARKETING",
  #   "MSW": "MASTER",
  #   "MSMS": "MASTERS",
  #   "MATH": "MATHEMATICS",
  #   "MECH": "MECHANIC",
  #   "MED": "MEDICAL",
  #   "MERP": "MEDICAL",
  #   "MSAA": "MEDICAL",
  #   "MEDCN": "MEICINE",
  #   "MEMBER": "MEMBE,",
  #   "METH": "METHODIST",
  #   "MICRO": "MICROSCOPE,",
  #   "MIN": "MINORITY",
  #   "MOL": "MOLECULAR",
  #   "MOM": "MO",
  #   "MULTIM": "MULTIMEDIA",
  #   "NATL": "NATIONAL",
  #   "NIH": "NATIONAL",
  #   "NGHBORHD": "NEIGHBORHOOD",
  #   "NEO": "NEONATAL",
  #   "PERI": "PERINATAL",
  #   "NW": "NETWORK",
  #   "NWBORN": "NEWBORN",
  #   "NGT": "NIGHT",
  #   "NMR": "NUCLEAR",
  #   "PRN": "NURSES",
  #   "NRSG": "NURSING",
  #   "OSCE": "OBJECTIVE",
  #   "OB": "OBSTETRICS",
  #   "GYN": "GYNECOLOGY",
  #   "OCCUP": "OCCUPATION",
  #   "OFF": "OFFICER",
  #   "ONCOL": "ONCOLOGY",
  #   "OPER": "OPERATOR",
  #   "OPTOM": "OPTOMETRIC",
  #   "ORTHO": "ORTHOPAEDIC",
  #   "OUTPAT": "OUTPATIENT",
  #   "PRNT": "PARENT",
  #   "P/T": "PART",
  #   "PRTNR": "PARTNER",
  #   "PATH": "PATHOLOGY",
  #   "PAT": "PATIENT",
  #   "PED": "PEDIATRICS",
  #   "PHOTOCOMP": "PHOTOCOMPOSITION",
  #   "PHOTOGRAPHR": "PHOTOGRAPHER",
  #   "PHYS": "PHYSICAL",
  #   "PHYSN": "PHYSICIAN",
  #   "PHYSIO": "PHYSIOLOGIST",
  #   "PL": "PLACEMENT",
  #   "PLAN": "PLANNING",
  #   "POS": "POINT",
  #   "PET": "POSITRON",
  #   "PRACT": "PRACTICE",
  #   "PRECRTN": "PRECERTIFICATION",
  #   "PRENA": "PRE-NATAL",
  #   "PREP": "PREPARATION",
  #   "PRESNT": "PRESENTATION",
  #   "PRES": "PRESIDENT",
  #   "PREV": "PREVENTIVE",
  #   "PRIN": "PRINCIPAL",
  #   "PROB": "PROBLEM",
  #   "PROC": "PROCESSING",
  #   "PRODU": "PRODUCER",
  #   "PROD": "PRODUCTION",
  #   "PROF": "PROFESSIONAL",
  #   "PROG": "PROGRAM,",
  #   "PROJ": "PROJECT",
  #   "PROM": "PROMOTIONS",
  #   "PSYCH": "PSYCHIATRY",
  #   "PSYCHOL": "PSYCHOLOGICAL",
  #   "PUBL": "PUBLICATION",
  #   "PDDS": "PUBLISHING",
  #   "PUR": "PURCHASING",
  #   "QUAL": "QUALITY",
  #   "QC": "QUALITY",
  #   "RADIO": "RADIOLOGY,",
  #   "REC": "RECEIVING",
  #   "RECRE": "RECREATION",
  #   "RECR": "RECRUITMENT",
  #   "REDRC": "REGIONAL",
  #   "REFRIG": "REFRIGERATION",
  #   "REG": "REGIONAL",
  #   "REGR": "REGISTRAR",
  #   "REHAB": "REHABILITATION",
  #   "REIMBURSE": "REIMBURSE,",
  #   "REL": "RELATION",
  #   "REP": "REPRESENTATIVE",
  #   "IDX": "REPRESENTS",
  #   "RES": "RESEARCH",
  #   "RESNET": "RESEARCH",
  #   "RESID": "RESIDENT,",
  #   "RESO": "RESOURCE",
  #   "RESP": "RESPIRATORY",
  #   "RCCC": "RILEY",
  #   "SAFE": "SAFETY",
  #   "SCAN": "SCANNING",
  #   "SCHED": "SCHEDULING,",
  #   "SCHL": "SCHOOL",
  #   "SCI": "SCIENCE",
  #   "SCNDRY": "SECONDARY",
  #   "SEC": "SECRETARY",
  #   "SECT": "SECTION",
  #   "SR": "SENIOR",
  #   "SER": "SERVICE",
  #   "SHIP": "SHIPPING",
  #   "SHOOT": "SHOOTING",
  #   "SBDC": "SOUTH",
  #   "SOC": "SOCIAL",
  #   "SEA": "SOUTHEAST",
  #   "SPEC": "SPECIAL,",
  #   "SPAN": "SPECIAL",
  #   "SPECT": "SPECTROMETRY",
  #   "SPON": "SPONSORED",
  #   "STF": "STAFF",
  #   "STATIS": "STATISTICAL",
  #   "STR": "STRESS",
  #   "TRDML": "TREADMILL",
  #   "STU": "STUDENT",
  #   "SRSC": "STUDENT",
  #   "STDY": "STUDY",
  #   "SUPT": "SUPERINTENDENT",
  #   "SUPR": "SUPERVISOR",
  #   "SUPP": "SUPPORT,",
  #   "SWIM": "SWIMMING",
  #   "SYS": "SYSTEM",
  #   "TCHNG": "TEACHING",
  #   "TECH": "TECHNICIAN",
  #   "TECHNOL": "TECHNOLOGIST",
  #   "TELC": "TELECOMMUNICATIONS",
  #   "TEMP": "TEMPORARY",
  #   "THTR": "THEATRE",
  #   "THER": "THERAPY",
  #   "TIS": "TISSUE",
  #   "TOX": "TOXICOLOGY",
  #   "TRNE": "TRAINEE",
  #   "TRNR": "TRAINER",
  #   "TRNG": "TRAINING",
  #   "TRANSCPT": "TRANSCRIPTIONST",
  #   "TRANS": "TRANSFER",
  #   "TRNSPLT": "TRANSPLANT",
  #   "TRANSPORT": "TRANSPORTATION",
  #   "ULTRA": "ULTRASOUND",
  #   "UG": "UNDERGRADUATE",
  #   "UNEMPLOY": "UNEMPLOYMENT",
  #   "UNIV": "UNIVERSITY",
  #   "UCET": "UNIVERSITY",
  #   "UMDA": "UNIVERSITY",
  #   "UPA": "UNIVERSITY",
  #   "UTIL": "UTILITIES",
  #   "VARS": "VARSITY",
  #   "VCLUB": "VARSITY",
  #   "VET": "VETERAN",
  #   "VC": "VICE",
  #   "VP": "VICE",
  #   "VIC": "VIRTUAL",
  #   "WAREHS": "WAREHOUSE",
  #   "WELLNS": "WELLNESS",
  #   "WCTR": "WELLS",
  #   "WSTRN": "WESTERN",
  #   "WKR": "WORKER",
  #   "AFG": "Afghanistan",
  #   "ALA": "Aland Islands",
  #   "ALB": "Albania",
  #   "DZA": "Algeria",
  #   "ASM": "American Samoa",
  #   "AND": "Andorra",
  #   "AGO": "Angola",
  #   "AIA": "Anguilla",
  #   "ATA": "Antarctica",
  #   "ATG": "Antigua and Barbuda",
  #   "ARG": "Argentina",
  #   "ARM": "Armenia",
  #   "ABW": "Aruba",
  #   "AUS": "Australia",
  #   "AUT": "Austria",
  #   "AZE": "Azerbaijan",
  #   "BHS": "Bahamas",
  #   "BHR": "Bahrain",
  #   "BGD": "Bangladesh",
  #   "BRB": "Barbados",
  #   "BLR": "Belarus",
  #   "BEL": "Belgium",
  #   "BLZ": "Belize",
  #   "BEN": "Benin",
  #   "BMU": "Bermuda",
  #   "BTN": "Bhutan",
  #   "BOL": "Bolivia",
  #   "BES": "Bonaire, Sint Eustatius and Saba",
  #   "BIH": "Bosnia and Herzegovina",
  #   "BWA": "Botswana",
  #   "BVT": "Bouvet Island",
  #   "BRA": "Brazil",
  #   "IOT": "British Indian Ocean Territory",
  #   "BRN": "Brunei Darussalam",
  #   "BGR": "Bulgaria",
  #   "BFA": "Burkina Faso",
  #   "BDI": "Burundi",
  #   "KHM": "Cambodia",
  #   "CMR": "Cameroon",
  #   "CAN": "Canada",
  #   "CPV": "Cape Verde",
  #   "CYM": "Cayman Islands",
  #   "CAF": "Central African Republic",
  #   "TCD": "Chad",
  #   "CHL": "Chile",
  #   "CHN": "China",
  #   "CXR": "Christmas Island",
  #   "CCK": "Cocos (Keeling) Islands",
  #   "COL": "Colombia",
  #   "COM": "Comoros",
  #   "COG": "Congo",
  #   "COD": "Congo, The Democratic Republic of ",
  #   "COK": "Cook Islands",
  #   "CRI": "Costa Rica",
  #   "CIV": "Cote d'Ivoire",
  #   "HRV": "Croatia",
  #   "CUB": "Cuba",
  #   "CUW": "Curaçao",
  #   "CYP": "Cyprus",
  #   "CZE": "Czechia",
  #   "DNK": "Denmark",
  #   "DJI": "Djibouti",
  #   "DMA": "Dominica",
  #   "DOM": "Dominican Republic",
  #   "ECU": "Ecuador",
  #   "EGY": "Egypt",
  #   "SLV": "El Salvador",
  #   "GNQ": "Equatorial Guinea",
  #   "ERI": "Eritrea",
  #   "EST": "Estonia",
  #   "ETH": "Ethiopia",
  #   "FLK": "Falkland Islands (Malvinas)",
  #   "FRO": "Faroe Islands",
  #   "FJI": "Fiji",
  #   "FIN": "Finland",
  #   "FRA": "France",
  #   "GUF": "French Guiana",
  #   "PYF": "French Polynesia",
  #   "ATF": "French Southern Territories",
  #   "GAB": "Gabon",
  #   "GMB": "Gambia",
  #   "GEO": "Georgia",
  #   "DEU": "Germany",
  #   "GHA": "Ghana",
  #   "GIB": "Gibraltar",
  #   "GRC": "Greece",
  #   "GRL": "Greenland",
  #   "GRD": "Grenada",
  #   "GLP": "Guadeloupe",
  #   "GUM": "Guam",
  #   "GTM": "Guatemala",
  #   "GGY": "Guernsey",
  #   "GIN": "Guinea",
  #   "GNB": "Guinea-Bissau",
  #   "GUY": "Guyana",
  #   "HTI": "Haiti",
  #   "HMD": "Heard and Mc Donald Islands",
  #   "VAT": "Holy See (Vatican City State)",
  #   "HND": "Honduras",
  #   "HKG": "Hong Kong",
  #   "HUN": "Hungary",
  #   "ISL": "Iceland",
  #   "IND": "India",
  #   "IDN": "Indonesia",
  #   "IRN": "Iran, Islamic Republic of",
  #   "IRQ": "Iraq",
  #   "IRL": "Ireland",
  #   "IMN": "Isle of Man",
  #   "ISR": "Israel",
  #   "ITA": "Italy",
  #   "JAM": "Jamaica",
  #   "JPN": "Japan",
  #   "JEY": "Jersey",
  #   "JOR": "Jordan",
  #   "KAZ": "Kazakstan",
  #   "KEN": "Kenya",
  #   "KIR": "Kiribati",
  #   "PRK": "Korea, Democratic People's Republic of",
  #   "KOR": "Korea, Republic of",
  #   "XKX": "Kosovo (temporary code)",
  #   "KWT": "Kuwait",
  #   "KGZ": "Kyrgyzstan",
  #   "LAO": "Lao, People's Democratic Republic",
  #   "LVA": "Latvia",
  #   "LBN": "Lebanon",
  #   "LSO": "Lesotho",
  #   "LBR": "Liberia",
  #   "LBY": "Libyan Arab Jamahiriya",
  #   "LIE": "Liechtenstein",
  #   "LTU": "Lithuania",
  #   "LUX": "Luxembourg",
  #   "MAC": "Macao",
  #   "MKD": "Macedonia, The Former Yugoslav Republic Of",
  #   "MDG": "Madagascar",
  #   "MWI": "Malawi",
  #   "MYS": "Malaysia",
  #   "MDV": "Maldives",
  #   "MLI": "Mali",
  #   "MLT": "Malta",
  #   "MHL": "Marshall Islands",
  #   "MTQ": "Martinique",
  #   "MRT": "Mauritania",
  #   "MUS": "Mauritius",
  #   "MYT": "Mayotte",
  #   "MEX": "Mexico",
  #   "FSM": "Micronesia, Federated States of",
  #   "MDA": "Moldova, Republic of",
  #   "MCO": "Monaco",
  #   "MNG": "Mongolia",
  #   "MNE": "Montenegro",
  #   "MSR": "Montserrat",
  #   "MAR": "Morocco",
  #   "MOZ": "Mozambique",
  #   "MMR": "Myanmar",
  #   "NAM": "Namibia",
  #   "NRU": "Nauru",
  #   "NPL": "Nepal",
  #   "NLD": "Netherlands",
  #   "NCL": "New Caledonia",
  #   "NZL": "New Zealand",
  #   "NIC": "Nicaragua",
  #   "NER": "Niger",
  #   "NGA": "Nigeria",
  #   "NIU": "Niue",
  #   "NFK": "Norfolk Island",
  #   "MNP": "Northern Mariana Islands",
  #   "NOR": "Norway",
  #   "OMN": "Oman",
  #   "PAK": "Pakistan",
  #   "PLW": "Palau",
  #   "PSE": "Palestinian Territory, Occupied",
  #   "PAN": "Panama",
  #   "PNG": "Papua New Guinea",
  #   "PRY": "Paraguay",
  #   "PER": "Peru",
  #   "PHL": "Philippines",
  #   "PCN": "Pitcairn",
  #   "POL": "Poland",
  #   "PRT": "Portugal",
  #   "PRI": "Puerto Rico",
  #   "QAT": "Qatar",
  #   "SRB": "Republic of Serbia",
  #   "REU": "Reunion",
  #   "ROU": "Romania",
  #   "RUS": "Russia Federation",
  #   "RWA": "Rwanda",
  #   "BLM": "Saint Barthélemy",
  #   "SHN": "Saint Helena",
  #   "KNA": "Saint Kitts & Nevis",
  #   "LCA": "Saint Lucia",
  #   "MAF": "Saint Martin",
  #   "SPM": "Saint Pierre and Miquelon",
  #   "VCT": "Saint Vincent and the Grenadines",
  #   "WSM": "Samoa",
  #   "SMR": "San Marino",
  #   "STP": "Sao Tome and Principe",
  #   "SAU": "Saudi Arabia",
  #   "SEN": "Senegal",
  #   "SYC": "Seychelles",
  #   "SLE": "Sierra Leone",
  #   "SGP": "Singapore",
  #   "SXM": "Sint Maarten",
  #   "SVK": "Slovakia",
  #   "SVN": "Slovenia",
  #   "SLB": "Solomon Islands",
  #   "SOM": "Somalia",
  #   "ZAF": "South Africa",
  #   "SGS": "South Georgia & The South Sandwich Islands",
  #   "SSD": "South Sudan",
  #   "ESP": "Spain",
  #   "LKA": "Sri Lanka",
  #   "SDN": "Sudan",
  #   "SUR": "Suriname",
  #   "SJM": "Svalbard and Jan Mayen",
  #   "SWZ": "Swaziland",
  #   "SWE": "Sweden",
  #   "CHE": "Switzerland",
  #   "SYR": "Syrian Arab Republic",
  #   "TWN": "Taiwan, Province of China",
  #   "TJK": "Tajikistan",
  #   "TZA": "Tanzania, United Republic of",
  #   "THA": "Thailand",
  #   "TLS": "Timor-Leste",
  #   "TGO": "Togo",
  #   "TKL": "Tokelau",
  #   "TON": "Tonga",
  #   "TTO": "Trinidad and Tobago",
  #   "TUN": "Tunisia",
  #   "TUR": "Turkey",
  #   "XTX": "Turkish Rep N Cyprus",
  #   "TKM": "Turkmenistan",
  #   "TCA": "Turks and Caicos Islands",
  #   "TUV": "Tuvalu",
  #   "UGA": "Uganda",
  #   "UKR": "Ukraine",
  #   "ARE": "United Arab Emirates",
  #   "GBR": "United Kingdom",
  #   "USA": "United States",
  #   "UMI": "United States Minor Outlying Islands",
  #   "URY": "Uruguay",
  #   "UZB": "Uzbekistan",
  #   "VUT": "Vanuatu",
  #   "VEN": "Venezuela",
  #   "VNM": "Vietnam",
  #   "VGB": "Virgin Islands, British",
  #   "VIR": "Virgin Islands, U.S.",
  #   "WLF": "Wallis and Futuna",
  #   "ESH": "Western Sahara",
  #   "YEM": "Yemen",
  #   "ZMB": "Zambia",
  #   "ZWE": "Zimbabwe"}
  # print(data_compare)
  filename2 = "csvjson.json"

  with open(filename2, 'r') as f:
    abbr_json = json.load(f)

    # fields = next(csvreader)
  abbr=[]
    # extracting each data row one by one
  for row in abbr_json:
    abbr.append(row)

  def bug_fixed(a):
    dict1 = {}
    for i in a:
      dict1[i['Abbreviation']] = i['Values']
    dict = json.dumps(dict1)
    return dict1
  abbr=bug_fixed(abbr)
  print(abbr)
  # with open(file_name) as f:
  #   data = json.load(f)
  # list_column=[]
  # for i in data:
  #     list_column.append(i[column_name])
  # dict = {}
  # pk=1
  # for i in list_column:
  #   lst1=[]
  #   for k in i.split(' '):
  #     # print(k)
  #     for j in range(len(abbr)):
  #       dict_matches = {}
  #       if re.sub(r"[^a-zA-Z0-9]+", ' ', abbr[j][0].lower()) == re.sub(r"[^a-zA-Z0-9]+", ' ',
  #                                                                              k.lower()):  # re.sub(r"[^a-zA-Z0-9]+", ' ', k)
  #         dict_matches[k]=abbr[j][1]
  #         lst1.append(dict_matches)
  #   if len(lst1)>0:
  #     dict[pk]=lst1
  #
  #   pk=pk+1
  return(dict)
column_name='lastName'
file_name='date_json.json'
a=standard_date(file_name,column_name)
print(a)
